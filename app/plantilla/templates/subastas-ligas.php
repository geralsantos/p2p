                ...
                <table>
                    ...
                    <tbody class="text-center">
                        <tr v-for="(row,index) in lista_jugadores">
                            ...
                            <td>{{row.altura}}</td>
                            <td>{{row.overall}}</td>
                            <td><a @click="iniciar_subasta(row,$event)"><button v-bind:title="row.estado_subasta" v-bind:style="{'background-color': row.color_estado_subasta,'width':'100%','color':'white'}" class="pd-setting-ed" style="width:100%" >{{row.estado_subasta}}</button></a></td>
                        </tr>
